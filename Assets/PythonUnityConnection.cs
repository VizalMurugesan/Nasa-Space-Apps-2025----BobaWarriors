...
public class PythonUnityConnector : MonoBehaviour
{
    ...
    public void RequestNextTick(int steps = 1)
    {
        if (!EnsureConnected())
        {
        ...
        if (metrics != null)
        {
            ...
            GameManager.Instance.timeManager.SetDayCount(tickCounter);
            GameManager.Instance.SetBarValues(metrics.soil_n, metrics.soil_moisture, metrics.yield_rate);
            //GameManager.Instance.SetTemperature(response.result.weather.current_json);
            string json = response.result.weather.current_json;
            var dict = JObject.Parse(json);

            // Convert to float or double before arithmetic
            float tmin = dict["TMIN"].Value<float>();
            float tmax = dict["TMAX"].Value<float>();

            float averageTemp = (tmin + tmax) / 2f;

            GameManager.Instance.SetTemperature(averageTemp);
            GameManager.Instance.SetWeather(response.result.weather.forecast[0]);
